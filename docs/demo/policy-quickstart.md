...
**For basic host blocking, policy.json alone is enough. Rego is optional for advanced rules (SSN, A2A headers, content patterns).**

## Policy Sources

| Source | Purpose |
|-------|---------|
| **policy.json** | Proxy baseline: `restricted_endpoints` loaded at startup. Blocks by hostname. No Rego required. Also used by core/crypto for BLAKE3 manifest signing. File-based, gitignored by default. Use `make setup` to create from policy.json.example. |
