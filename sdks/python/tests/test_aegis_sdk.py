...
from aegis_sdk import Aegis, AegisClient, AegisResult
...
class FakeClient:
    ...
    def verify(self, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None) -> AegisResult:
        ...
        return AegisResult(payload, 200, {"valid": True})


def _new_aegis(tmp_path: Path) -> Aegis:
    return Aegis(
...
def test_client_uses_env_base_url(monkeypatch) -> None:
    monkeypatch.setenv("AEGIS_BASE_URL", "http://localhost:9000")
    client = AegisClient(base_url=None)
    assert client.base_url == "http://localhost:9000"


def test_identity_headers_present(tmp_path: Path) -> None:
    sdk = _new_aegis(tmp_path)
    try:
        headers = sdk._identity_headers()
        assert headers["X-Aegis-Session-Id"] == "sess-1"
        assert headers["X-Aegis-User-Id"] == "user-1"
        assert headers["X-Aegis-IAM-Role"] == "auditor"
    finally:
        sdk.close()
